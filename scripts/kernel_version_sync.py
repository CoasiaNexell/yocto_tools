...
class KernelVersionSync:
    ...
    def __init__(self, bbappendPath, opteePath, rootPath, kernelDirName):
        self.bbappendPath = bbappendPath
        self.opteePath = opteePath
    ...
    def getKernelMakeFile(self) :
        if os.path.exists(self.kernelSrcPath + "/Makefile") :
            ...
            print ("kernel source check please, No exist Makefile in Kernel Source ")

    def setBBAPPENDfile(self):
        if os.path.exists(self.bbappendPath):
        ...
        if os.path.exists(self.opteePath):
            print ("exist " + self.opteePath)
            with open(self.opteePath, 'a') as f:
                f.write("\n")
                f.write('PATH_KERNEL_SRC := "' + self.kernelSrcPath + '"')
        else:
            print ("optee bbappend file does not exist")


def main(bbappendPath, opteePath, rootPath, kernelDirName):
    kvs = KernelVersionSync(bbappendPath, opteePath, rootPath, kernelDirName)
